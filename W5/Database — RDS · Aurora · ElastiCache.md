...
> #### 📌 RPO / RTO 란? (재해 복구 핵심 지표)
>
> 재해 복구(DR) 설계에서 가장 기본이 되는 두 지표. 둘 다 "장애가 났을 때 얼마나 손해를 감수할 것인가"를 정의하는 **목표값**이다.
>
> **RPO (Recovery Point Objective) — 복구 시점 목표**
> - **"데이터를 얼마나 잃어도 되는가"** = 허용 가능한 **데이터 손실량**을 시간으로 표현
> - 장애 직전 마지막으로 안전하게 보존된 데이터 시점 ↔ 장애 발생 시점 사이의 간격. 복제·백업이 자주 일어날수록 RPO는 작아진다.
> - `RPO 1초` → 장애가 나도 **최근 1초치 데이터만 유실** 가능. 거의 손실 없는 수준
> - 예: 백업을 1시간마다 받으면 RPO는 최대 1시간 (직전 1시간 작업분이 날아갈 수 있음)
>
> **RTO (Recovery Time Objective) — 복구 시간 목표**
> - **"얼마나 빨리 복구해야 하는가"** = 허용 가능한 **다운타임**을 시간으로 표현
> - 장애 발생 시점 ↔ 서비스가 정상 가동될 때까지의 목표 시간
> - `RTO 1분 미만` → 장애 후 **1분 안에 서비스가 복구**되어야 함
>
> **한 줄 정리**: RPO = **데이터 손실 허용 한계**(과거를 본다), RTO = **복구 시간 허용 한계**(미래를 본다).
>
> ```
> 마지막 백업/복제 ──RPO──▶ [장애 발생] ──RTO──▶ 서비스 복구 완료
>      (잃는 데이터, 과거)              (복구까지 걸리는 시간, 미래)
> ```
...
>
> 보기에 RPO/RTO 수치가 흩어져 나오면 그 숫자로 서비스를 고르게 한다. "RTO 수 시간 허용"이면 스냅샷 복원으로 충분하지만, "RTO 1분" 수준이면 **Aurora Global DB**나 동기식 복제 기반 구성을 골라야 한다.
...
- **RPO** = 데이터 손실 허용 한계(과거) / **RTO** = 복구 시간 허용 한계(미래)
